...
# GUI setup
root = tk.Tk()
root.title("Currency Converter - Welcome")
...
headlabel = tk.Label(Tops, font=('lato black', 19, 'bold'),
                     text='Currency Converter : Welcome',
                     bg='#e6e5e5', fg='black')
...
Button(root, font=('arial', 15, 'bold'), text="Convert", padx=2, pady=2,
       bg="grey", fg="white", command=RealTimeCurrencyConversion).grid(row=6, column=0)

Button(root, font=('arial', 15, 'bold'), text="Clear All", padx=2, pady=2,
       bg="grey", fg="white", command=clear_all).grid(row=10, column=0)
